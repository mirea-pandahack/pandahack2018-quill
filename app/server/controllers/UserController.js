...
moment.locale('ru'); //@TODO do we need it here?
...
/**
 * Send an email invitation email given a user id.
 */
UserController.sendInvitationEmailById = function (id, callback) {
  User.findOne(
    {
      _id: id
    },
    function (err, user) {
      ...
      Mailer.sendInvitationEmail(user.email);
      return callback(err, user);
    });
};
